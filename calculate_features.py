...
def get_outcome(last_price_list, tolerance=0.01):
    ...
    print(
        f"Resolutions reconstructed with tolerance: {tolerance}. Implies outcome 0 if last price of 0 is less than {1 - tolerance}, outcome 1 if last price more than {0+tolerance}. \n{num_nas} prices were outside of tolerance."
    )
...
def prep_price_data(price_df, meta_data_df):
    ...
    # make outcome column
    meta_data_df["resolution"] = get_outcome(
        meta_data_df.lastTradePrice, tolerance=0.015
    )
...
def main():
    ...
    prices = prices.assign(
        brier=calc_brier(p=prices["p"], token_correct=prices["token_correct"])
    )
    prices = prices.assign(
        abs_error=abs_error(p=prices["p"], token_correct=prices["token_correct"])
    )
    ...
    for market_id in tqdm(
        prices["market_id"].unique(), desc="Calculating features for all markets."
    ):
